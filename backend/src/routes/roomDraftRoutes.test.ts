...
describe('Player Draft API', () => {
  ...
  describe('PUT /api/rooms/:roomId/player/:playerId/draft (TI-01)', () => {
    it('deve atualizar o rascunho de pontuação do jogador e retornar 200', async () => {
      ...
      const response = await request(app)
        .put(`/api/rooms/${roomId}/player/${playerId}/draft`)
        .send(draftPayload);

      expect(response.status).toBe(200);
      const updatedPlayer = response.body.players.find((p: any) => p.id === playerId);
      expect(updatedPlayer.roundDraft).toEqual(draftPayload);
    });
    ...
    it('deve retornar 404 se a sala ou o jogador não existirem', async () => {
      ...
      const resInvalidRoom = await request(app)
        .put('/api/rooms/invalid-room/player/p1/draft')
        .send(draftPayload);
      ...
      const resInvalidPlayer = await request(app)
        .put(`/api/rooms/${roomId}/player/invalid-player/draft`)
        .send(draftPayload);
  ...
  describe('Sequência de atualização de draft e finalização de rodada (TI-02)', () => {
    it('deve atualizar draft sequencialmente e consolidar no finishRound', async () => {
      ...
      await request(app)
        .put(`/api/rooms/${roomId}/player/${p1Id}/draft`)
        .send({ selectedNumbers: [10], selectedBonus: [], isMultiplierActive: false, total: 20 });

      await request(app)
        .put(`/api/rooms/${roomId}/player/${p1Id}/draft`)
        .send({ selectedNumbers: [10, 5], selectedBonus: [10], isMultiplierActive: true, total: 45 });

      await request(app)
        .put(`/api/rooms/${roomId}/player/${p2Id}/draft`)
        .send({ selectedNumbers: [5, 5], selectedBonus: [], isMultiplierActive: false, total: 10 });
      ...
      const finalP1 = finishRes.body.players.find((p: any) => p.id === p1Id);
      const finalP2 = finishRes.body.players.find((p: any) => p.id === p2Id);

      expect(finalP1.score).toBe(45);
      expect(finalP2.score).toBe(10);
      expect(finalP1.roundDraft).toBeUndefined();
      expect(finalP2.roundDraft).toBeUndefined();
    });
  });
});
