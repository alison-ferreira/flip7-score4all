...
export default function ScoreKeypad({ player, onConfirm, onCancel }) {
  const [selectedNumbers, setSelectedNumbers] = useState(new Set())
  const [selectedBonus, setSelectedBonus] = useState(new Set())
  ...
  const getCalculation = () => {
    ...
    let calculatedNumberSum = numSum

    if (numArr.length > 0) {
      const numText = numArr.join('+')
      if (isMultiplierActive) {
        calculatedNumberSum = numSum * 2
    ...
    let bonusSum = bonusArr.reduce((acc, val) => acc + val, 0)
    ...
    <div className="modal-overlay">
      <div className="modal-content">
        <div style={{textAlign: 'center', marginBottom: 15}}>
          <h3 style={{fontSize: 22, color: 'var(--accent)'}}>{player.name}</h3>
        </div>
